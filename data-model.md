...
| --- | --- | --- |
| `users/{uid}` | `email`, `displayName`, `role` (`admin`\|`staff`\|`partner`), `menus` (`"all"` or `string[]`), `active`, `createdAt`, `inviteId?`, `onboardingComplete?`, `contactName?`, `phone?`, `address? {ville, commune, quartier, repere?}`, `idNumber?`, `pointDeVente?`, `verified?`, `poste?` | `uid` matches the Firebase Auth UID. `inviteId` is only present on admin/staff accounts created via `/join` (see `invites` below); an audit trail, not read by any rule. The `onboardingComplete` through `verified` fields are partner-only. `onboardingComplete` through `pointDeVente` are written by the guided onboarding wizard — see below. `verified` (sprint 16) defaults `false` at signup and is flipped by admin from the dashboard's "Boutiques partenaires" card after a phone-call confirmation — informational only, never a gate on ordering. `poste` (sprint 17, staff-only) is `"Directeur de Production"` \| `"Chargée de Commercialisation"` \| `"Personnalisé"` — see below |
